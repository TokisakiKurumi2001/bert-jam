...
class BertTransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--from-tf', action='store_true',
                            help='load bert weights from tf checkpoints')
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return BertTransformerEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return BertTransformerDecoder(args, tgt_dict, embed_tokens)
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, bert_input, **kwargs):
        ...
        """
        bert_encoder_padding_mask = bert_input.eq(self.berttokenizer.pad_token_id)
        # _, _, bert_encoder_out =  self.bert_encoder(bert_input, attention_mask= ~bert_encoder_padding_mask)
        # bert_encoder_out = bert_encoder_out[1:]
        # bert_encoder_out = bert_encoder_out[self.bert_output_layer]
        # bert_encoder_out = bert_encoder_out.permute(1,0,2).contiguous()
        bert_encoder_out = self.forward_bert(bert_input, bert_encoder_padding_mask)
        if self.mask_cls_sep:
            bert_encoder_padding_mask += bert_input.eq(self.berttokenizer.cls_token_id)
            bert_encoder_padding_mask += bert_input.eq(self.berttokenizer.sep_token_id)
        bert_encoder_out = {
    ...
    def forward_bert(self, bert_input, bert_encoder_padding_mask):
        _, _, bert_encoder_out = self.bert_encoder(bert_input, attention_mask=~bert_encoder_padding_mask)
        bert_encoder_out = bert_encoder_out[1:]
        bert_encoder_out = bert_encoder_out[self.bert_output_layer]
        bert_encoder_out = bert_encoder_out.permute(1,0,2).contiguous()
        return bert_encoder_out
...
class BertTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.layers.extend([
            BertTransformerEncoderLayer(args, bert_gate=bert_gates[i])
...
class BertTransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if decoder_no_bert:
            ...
            self.layers.extend([
                BertTransformerDecoderLayer(args, no_encoder_attn, bert_gate=bert_gates[i])
                for i in range(args.decoder_layers)
            ])
...
class BertTransformerDecoderLayer(nn.Module):
    """Decoder layer block.
...
@register_model_architecture('bert_transformer', 'bert_transformer')
...
@register_model_architecture('bert_transformer', 'bert_transformer_iwslt_de_en')
def bert_transformer_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
...
@register_model_architecture('bert_transformer', 'bert_transformer_wmt_en_de_big')
def bert_transformer_wmt_en_de_big(args):
